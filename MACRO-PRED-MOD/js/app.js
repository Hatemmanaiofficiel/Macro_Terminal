/**
 * MACROFORECAST Master Application 12V (SOVEREIGN_FREEDOM)
...
function logEvent(msg) {
    ...
    item.innerHTML = `<span style="color:#ffffff">></span> <span style="color:#ffffff">${msg}</span>`;
    log.prepend(item);
    ...
    if (state.activePage === 1) {
        html = `
            <div class="fade-in">
                ...
                <div style="font-size:10px; color:#ffffff; line-height:1.5; margin-bottom:15px; text-align:justify; opacity:0.8;">
                    Institutional analysis for ${state.countryName} indicates a phase of <b style="color:var(--accent-green)">${state.momentum > 0 ? "BULLISH_ACCUMULATION" : "BEARISH_CONTRACTION"}</b>. 
                    The calculated Risk Index of ${state.riskScore} suggests <span style="color:${state.riskScore > 50 ? 'var(--accent-red)' : 'var(--accent-green)'}">${state.riskScore > 50 ? "elevated exposure risk" : "stable underlying architecture"}</span> within the current macro-cycle.
                </div>
        ...
        html = `
            <div class="fade-in">
                ...
                <div style="border:1px solid var(--border-nexus); padding:12px; background:#001229;">
                    <div style="font-size:8px; color:#ffffff; text-transform:uppercase; letter-spacing:1px;">Confidence Layer: ${state.validity}%</div>
                    <div style="height:3px; background:var(--border-nexus); margin-top:8px;">
                        <div style="height:100%; background:var(--accent-green); width:${state.validity}%;"></div>
                    </div>
                </div>
            </div>
        `;
        ...
        html = `
            <div class="fade-in">
                ...
                <div style="margin-top:20px; border:1px solid var(--border-nexus); padding:12px; background:#001229;">
                    <div style="font-size:8px; color:#ffffff; text-transform:uppercase; margin-bottom:5px;">System Security Hash</div>
                    <div style="font-family:var(--font-mono); font-size:8px; color:var(--accent-green); word-break:break-all; opacity:0.6;">
                        NEXUS_SOLVER_RUN_${Math.random().toString(16).slice(2, 10).toUpperCase()}_LOG
                    </div>
                </div>

                <div class="nexus-seal" style="margin-top:20px; padding:10px; text-align:center; font-size:8px; color:#ffffff; font-family:var(--font-mono); letter-spacing:2px; opacity:0.5;">
...
function generateStrategicAdvisory(s) {
    ...
    const strategies = {
        ...
        NEUTRAL: [
            ...
            "1. PURE_ALPHA_PURSUIT: Focus on micro-divergences within the ${s.countryName} block.<br>2. CORRELATION_TEST: Verify sector links before increasing exposure."
        ]
    };
...
function renderRadarChart() {
    ...
    radarChart = new Chart(ctx, {
        type: 'radar', data: { labels: l, datasets: [{ data: d, backgroundColor: 'rgba(0,229,255,0.1)', borderColor: 'var(--accent-yellow)', borderWidth: 1.5, pointRadius: 2, pointBackgroundColor: 'var(--accent-yellow)' }] },
        options: {
            responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } },
            scales: { r: { angleLines: { color: 'var(--border-nexus)' }, grid: { color: 'var(--border-nexus)' }, pointLabels: { color: '#ffffff', font: { family: 'var(--font-mono)', size: 8, weight: 'bold' } }, ticks: { display: false }, min: 0, max: 10 } }
        }
    });
}
...
function renderMainChart(history, forecast, paths) {
    ...
    mainChart = new Chart(ctx, {
        ...
        options: {
            ...
            scales: {
                x: { grid: { color: 'var(--border-nexus)' }, ticks: { color: '#ffffff', font: { size: 9, family: 'var(--font-mono)' } } },
                y: { grid: { color: 'var(--border-nexus)' }, ticks: { color: '#ffffff', font: { size: 9, family: 'var(--font-mono)' } } }
            }
        }
    });
}
...
function renderMatrix() {
    ...
    inds.forEach(a => {
        inds.forEach(b => {
            ...
            const color = c > 0.4 ? 'var(--accent-green)' : (c < -0.4 ? 'var(--accent-red)' : 'var(--accent-green)');
            html += `
...
function compileInstitutionalReport() {
    ...
    const terminalId = `NEXUS-12V-` + Math.random().toString(16).slice(2, 8).toUpperCase();
        ...
        fullHtml += `
            <div class="print-report-page">
                ...
                <footer class="print-footer">
                    CONFIDENTIAL PREVIEW | PROBABILISTIC MACRO-SOLVER | TERMINAL_S_12V_FREEDOM
                </footer>
            </div>
        `;
